...
Chain.prototype._open = async function _open() {
  this.options.bip91 = true; // Force Segwit2x
  ...
  if (this.options.bip91)
    this.logger.warning('BIP91 enabled. Segwit2x will be enforced.');
...
Chain.prototype.getDeployments = async function getDeployments(time, prev) {
  ...
  // segwit2x is now enforced (bip91).
  if (this.options.bip91) {
    if (witness === thresholdStates.STARTED) {
      if (await this.isActive(prev, deployments.segwit2x))
        state.bip91 = true;
    }
  }
